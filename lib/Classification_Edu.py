...
class MMClassification:
    def __init__(
        ...
        self.config = os.path.join(
            self.file_dirname, 'models', 'LeNet/LeNet.py')
        self.checkpoint = os.path.join(
            self.file_dirname, 'models', 'LeNet/LeNet.pth')
        ...
        if not self.save_fold:
            ...
            if not save_fold:
                self.save_fold = os.path.join(
                    self.cwd, 'checkpoints/cls_model')
    ...
    def inference(self, device='cpu',
        ...
        if not checkpoint:
            checkpoint = os.path.join(
                self.cwd, 'checkpoints/cls_model/hand_gray/latest.pth')
        ...
        results = []
        if (image[-1] != '/'):
            img_array = mmcv.imread(
                image, flag='grayscale' if self.backbone == "LeNet" else 'color')
            result = inference_model(model, img_array)  # 此处的model和外面的无关,纯局部变量
            model.show_result(image, result, show=show,
                              out_file=os.path.join(save_fold, image))
            chinese_res = []
            tmp = {}
            ...
            for i, img in enumerate(tqdm(os.listdir(img_dir))):
                # 此处的model和外面的无关,纯局部变量
                result = inference_model(model, img_dir + img)
    ...
    def load_dataset(self, path):
        ...
        self.cfg.data.train.data_prefix = os.path.join(
            self.dataset_path, 'training_set')
        self.cfg.data.train.classes = os.path.join(
            self.dataset_path, 'classes.txt')

        self.cfg.data.val.data_prefix = os.path.join(
            self.dataset_path, 'val_set')
        self.cfg.data.val.ann_file = os.path.join(self.dataset_path, 'val.txt')
        self.cfg.data.val.classes = os.path.join(
            self.dataset_path, 'classes.txt')

        self.cfg.data.test.data_prefix = os.path.join(
            self.dataset_path, 'test_set')
        self.cfg.data.test.ann_file = os.path.join(
            self.dataset_path, 'test.txt')
        self.cfg.data.test.classes = os.path.join(
            self.dataset_path, 'classes.txt')
