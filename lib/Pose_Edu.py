...
class MMPose:
        ...
        for item in ckpt_cfg_list:
            if item[-1] == 'y':
                self.det_config = os.path.join(
                    CUR_PATH, backbone_det_path, item)
            elif item[-1] == 'h':
    ...
    def inference(self,
        ...
        pose_model = init_pose_model(
            self.pose_config, self.pose_checkpoint, device=device)
        # initialize detector
        det_model = init_detector(
            self.det_config, self.det_checkpoint, device=device)
    ...
    def load_dataset(self, path):
        ...
        self.cfg.data.train.ann_file = os.path.join(
            self.dataset_path, 'train.json')
        self.cfg.data.train.img_prefix = os.path.join(
            self.dataset_path, 'images/')

        self.cfg.data.val.type = 'PoseDataset'
        self.cfg.data.val.ann_file = os.path.join(
            self.dataset_path, 'val.json')
        self.cfg.data.val.img_prefix = os.path.join(
            self.dataset_path, 'images/')

        self.cfg.data.test.type = 'PoseDataset'
        self.cfg.data.test.ann_file = os.path.join(
            self.dataset_path, 'val.json')
        self.cfg.data.test.img_prefix = os.path.join(
            self.dataset_path, 'images/')
